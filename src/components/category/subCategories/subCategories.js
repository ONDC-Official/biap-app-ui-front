...
import { PRODUCT_SUBCATEGORY, categoryList } from "../../../constants/categories";
...
const SubCategories = () => {
    ...
    const getAllOffers = async () => {
        ...
        try {
            ...
            const findCategory = categoryName ? categoryList.find((item) => item.shortName === categoryName) : null;
            const domain = findCategory ? findCategory.domain : '';
            const data = await cancellablePromise(getAllOffersRequest(domain, lat, lng));
            setOffers(data);
        } catch (err) {
            dispatch({
